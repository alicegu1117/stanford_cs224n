    ...
    def parse_step(self, transition):
        ...
        if transition == "S":
            ...
            if transition == "LA":
                ...
                dependency_tuple = (head, dependent)
                self.dependencies.append(dependency_tuple)
            else:
                dependent= self.stack.pop(-1)
                head = self.stack.pop(-1)
                self.stack.append(head)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        if len(unfinished_parses) < batch_size:
            batch_size = len(unfinished_parses) # handle out-of-bound cases
        # print(f"{batch_size=}")
        ...
        for idx, minibatch in enumerate(minibatches):
            # print(f"{idx=}")
            # print(f"{minibatch.buffer=}")
            # print(f"{minibatch.stack=}")
            minibatch.parse_step(transitions[idx])  # CAUTION!! don't use parse because it expects a list therefore will split 'LA' as 'L' and 'A'
            if len(minibatch.buffer) == 0 and len(minibatch.stack) == 1:
                unfinished_parses.remove(minibatch)  #pop will not free underlying memory because there's another reference from partial_parse
    
    for idx, partial_parse in enumerate(partial_parses):
        dependencies.insert(idx, partial_parse.dependencies)

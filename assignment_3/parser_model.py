...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # Step 1: Use torch.gather to gather embeddings based on indices in w
        batch_size, n_features = w.size(0), w.size(1)
        w_reshape = w.view(-1) #shape to 1-dimensional
        gathered_embeddings = torch.gather(self.embeddings, 0, w_reshape.unsqueeze(1).expand(-1, self.embed_size)) #input tensor and index tensor needs to have the same num of dimensions. For dimension d != dim, index.size(d) needs to be < input.size(d)
        # Step 2: Reshape the tensor if necessary
        x = gathered_embeddings.view(batch_size, n_features * self.embeddings.shape[1])
    ...
    def forward(self, w):
        ...
        # print(f"{batch_size=}")
        # print(f"{n_features=}")
        # print(f"{self.embed_size=}")
        embedded = self.embedding_lookup(w) # b x (n * e)
        hidden = torch.matmul(embedded, self.embed_to_hidden_weight) + self.embed_to_hidden_bias

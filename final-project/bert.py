...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # all key, query, value are of shape [bs, num_attention_heads, seq_len, attention_head_size]
    S = torch.matmul(query, key.transpose(-1, -2))  # [bs, num_attention_heads, seq_len, seq_len]
    S += attention_mask
    ...
    S = S.softmax(dim=-1) # [bs, num_attention_heads, seq_len, seq_len]
    V = torch.matmul(S, value)  # [bs, num_attention_heads, seq_len, attention_head_size]
    V = V.permute(0, 2, 1, 3).contiguous()  # [bs, seq_len, num_attention_heads, attention_head_size]
    V = V.view(V.size(0), V.size(1), self.all_head_size)  # [bs, seq_len, hidden_size]
    return V
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    after_dense = dense_layer(output)
    after_dropout = dropout(after_dense)
    after_add = input + after_dropout
    after_norm = ln_layer(after_add)
    return after_norm
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    """
    after_attention = self.self_attention(hidden_states, attention_mask)
    after_add_norm = self.add_norm(hidden_states, after_attention, self.attention_dense, self.attention_dropout, self.attention_layer_norm)
    after_ff = self.interm_af(self.interm_dense(after_add_norm))
    after_add_norm = self.add_norm(after_add_norm, after_ff, self.out_dense, self.out_dropout, self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    pos_embeds = None
    pos_embeds = self.pos_embedding(pos_ids)  # (input_shape, hidden_size)
    ...
    final_embeds = self.embed_layer_norm(final_embeds)
    final_embeds = self.embed_dropout(final_embeds)
    return final_embeds

...
if args.function == 'pretrain':
    ...
    if __name__ == "__main__":
        trainer = trainer.Trainer(model, pretrain_dataset, None, trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=args.pretrain_lr, lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size, num_workers=4, writer=writer, ckpt_path=args.checkpoint_path))
        trainer.train()
    ...
    if __name__ == "__main__":
        params = {}
        params['max_epochs'] = 75
        params['batch_size'] = 256
        params['learning_rate'] = args.finetune_lr
        params['lr_decay'] = True
        params['warmup_tokens'] = 512 * 20
        params['final_tokens'] = 200 * pretrain_dataset.vocab_size * pretrain_dataset.block_size
        ...
        finetune_dataset = dataset.NameDataset(pretrain_dataset, data) 

        trainer = trainer.Trainer(model, finetune_dataset, None, trainer.TrainerConfig(**params))
        trainer.train()

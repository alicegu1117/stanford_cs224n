...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.dropout = nn.Dropout(p=0,inplace=False)    # unfortunately, seems like the test is expecting dropout to be no-op
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        target_masks = (target_padded != self.vocab.tgt['<pad>']).float()   # mask used to zero out padding tokens

        # Compute log probability of generating true target words
        target_gold_words_log_prob = torch.gather(P, index=target_padded[1:].unsqueeze(-1), dim=-1).squeeze(
            -1) * target_masks[1:]  # gather predicted log probabilities by indices of target subwords. 
        scores = target_gold_words_log_prob.sum(dim=0)  # sum the log probabilities of the target words across all timesteps. Effectively computing cross entropy loss. 
        return scores
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded) # comments are confusing. source_padded is a tensor of indices rather than words 
        X = X.permute(1, 2, 0)  # (b, e, src_len)
        X_cnn = self.post_embed_cnn(X)  # (b, e, src_len)
        X_cnn  = X_cnn.permute(2, 0, 1) # back to (src_len, b, e)
        packed_X = pack_padded_sequence(X_cnn, source_lengths, enforce_sorted=True)   # packs a padded sequence into a packed sequence

        h0 = torch.zeros(2, X_cnn.size(1), self.hidden_size).to(X.device)   # move h0 to the same device as X in order to perform operations on them together, otherwise runtime error
        c0 = torch.zeros(2, X_cnn.size(1), self.hidden_size).to(X.device) # (2, b, h)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(packed_X, (h0, c0)) # (src_len, b, h*2), (2, b, h*2)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens, batch_first=False)    # converts a packed sequence to a padded sequence
        enc_hiddens = enc_hiddens.permute(1, 0, 2)  # (b, src_len, h*2)

        init_decoder_hidden = self.h_projection(torch.cat((last_hidden[0], last_hidden[1]), 1)) # (b, 2*h) @ (2*h, h)
        init_decoder_cell = self.c_projection(torch.cat((last_cell[0], last_cell[1]), 1)) # (b, 2*h) @ (2*h, h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        str_len = enc_hiddens.size(1)
        enc_hiddens_proj = self.att_projection(enc_hiddens) # (b, src_len, 2h) @ (2h, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden.unsqueeze(2)).squeeze(2) # (b, src_len, h) bmm (b, h, 1) -> (b, src_len, 1) -> (b, src_len)
        ### END YOUR CODE
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1) # (b, 1, src_len) bmm (b, src_len, 2h) -> (b, 1, 2h) -> (b, 2h)
